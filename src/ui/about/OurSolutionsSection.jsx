...
const DifferentiatingEdge = () => {
  ...
  const getDisplayText = (description, index) => {
    ...
    const restOfText = description.substring(firstSentenceEnd + 1);
        ...
        <div className="relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((feature, index) => {
              const textParts = getDisplayText(feature.description, index);
              ...
              return (
                ...
                >
                  ...
                  <div className="text-blue-100 text-sm sm:text-base leading-relaxed">
                    {textParts.firstSentence}
                    {textParts.restOfText && (
                      <>
                        {isExpanded && (
                          <span> {textParts.restOfText}</span>
                        )}
                        <button
                          onClick={() => toggleExpand(index)}
                          className="ml-2 text-cyan-400 hover:text-cyan-300 font-medium underline transition-colors"
                        >
